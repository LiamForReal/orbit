...
private:
	...
	SOCKET& _socket;								
	std::map<unsigned int, std::pair<ECDHE, uint256_t>> _ecdheInfo; //circuit id to pair of ecdhe main object and seconed only a saver of tmpKey 
												   //public key, product
	...
	std::map<unsigned int, uint256_t>& _aesKeys; // circuit id to aes keys 
	RequestResult rr;
};